...
public final class LaunchEQEndpoint implements CTPEndpoint {
  ...
  /**
   * Redirects the caller to start EQ for the supplied case.
   * If there is no user signed in then the G-Suite IDP will require them to sign in 
   * before control is passed to this endpoint.
   *
  ...
  public ResponseEntity<String> launchEQ(@PathVariable("caseId") final UUID caseId, @SAMLUser SAMLUserDetails user) throws CTPException {
    log.with("pathParam", caseId)
    .with("user", user.getUsername())
    .info("Entering launchEQ");
    
    // To prove redirection build a test url. This is based on the data available to prove it's dynamic.
